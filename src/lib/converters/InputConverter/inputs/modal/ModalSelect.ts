...
import { InputTypeValue, ModalField, SelectOption, TranslatedString } from "../../../../../types/common";
...
export default class ModalSelect {
    ...
    public render() {
        ...
        return html`<div class="form-control w-full">
            ...
            <select class="select select-bordered" name=${this.name} ?required=${this.required}>
                <option selected disabled>${__("Please select an option")}</option>
                ${this.data.map(
                    (datum) =>
                        html`<option value=${datum.id} ?selected=${datum.id === this.value}>${datum.name}</option>`,
                )}
            </select>
        </div>`;
    }
