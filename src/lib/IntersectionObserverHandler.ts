...
} & ({ intersecting: { do: () => void } } | { intersected: { do: () => void } });
...
export class IntersectionObserverHandler {
    ...
    public init() {
        if (this.intersecting.ref instanceof HTMLElement && this.intersected.ref instanceof HTMLElement) {
            this.observer = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    if (entry.target === this.intersecting.ref && entry.isIntersecting && entry.intersectionRatio > 0) {
