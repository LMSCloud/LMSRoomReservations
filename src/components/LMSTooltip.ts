...
export default class LMSTooltip extends LitElement {
    ...
    @property({ type: String, attribute: "data-placement" }) placement: "top" | "bottom" | "left" | "right" = "bottom";
    ...
    override willUpdate(changedProperties: PropertyValues<this>) {
        if (changedProperties.has("target")) {
            const previousTarget = changedProperties.get("target") as HTMLElement | null;
            ...
            switch (this.placement) {
                case "top":
                    ...
                    this.tooltip.style.left = `${
                        window.scrollX + targetRect.left + (targetRect.width - this.tooltip.offsetWidth) / 2
                    }px`;
                    break;
                case "bottom":
                ...
                case "left":
                    this.tooltip.style.top = `${
                        window.scrollY + targetRect.top + (targetRect.height - this.tooltip.offsetHeight) / 2
                    }px`;
