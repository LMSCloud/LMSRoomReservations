...
export default class LMSDataNavbar extends LitElement {
    ...
    private setStateDrawer(transform: string, opacity: string, visibility: string) {
    ...
    override render() {
        return html`
            <nav class="navbar mb-4 max-w-min rounded-xl bg-base-100 sm:hidden sm:w-full">
            ...
            <nav
                id="navbar-drawer"
                class="navbar fixed bottom-0 left-0 z-[1050] mb-4 min-h-fit w-full min-w-min flex-col gap-4 overflow-y-scroll rounded-t-xl bg-base-100 p-8 transition-all duration-200 sm:min-h-16 sm:static sm:flex sm:flex-row sm:overflow-y-visible sm:rounded-xl sm:p-4"
            >
                <div class="p-4 sm:hidden">
                    <h2 class="text-lg font-bold">${__("Table Controls")}</h2>
                </div>
                <button class="btn btn-ghost absolute right-0 top-0 m-2 sm:hidden" @click=${this.handleClick}>
                    ${litFontawesome(faTimes, {
                        className: "w-4 h-4 inline-block",
                    })}
                </button>
                <div class="w-full justify-start justify-center sm:navbar-start sm:w-1/2">
                    <slot name="navbar-start"></slot>
                </div>
                <div class="w-full justify-center sm:navbar-center sm:w-auto">
                    <slot name="navbar-center"></slot>
                </div>
                <div class="w-full justify-end justify-center sm:navbar-end sm:w-1/2">
