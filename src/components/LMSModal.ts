import { faArrowRight, faClose, faPlus, faXmarkCircle } from "@fortawesome/free-solid-svg-icons";
...
export default class LMSModal extends LitElement {
    ...
    @state() protected modalTitle: string | DirectiveResult<typeof TranslateDirective> = "";
    ...
    private boundHandleKeyDown = (e: KeyboardEvent) => this.handleKeyDown.bind(this)(e);
    ...
    private async create(e: Event) {
        e.preventDefault();
        const { endpoint } = this.createOpts;
        const response = await requestHandler.post(endpoint, {
            ...this.fields.reduce((acc, field) => ({ ...acc, [field.name]: field.value }), {}),
        });
                ...
                this.alert = {
                    ...
                    message: Array.isArray(result.error)
                        ? html`<span>Sorry!</span>
                              <ol>
                                  ${map(result.error, (message: string) => html`<li>${message}</li>`)}
                              </ol>`
    ...
    protected composeTaggedInputs(field: ModalField, taggedData?: TaggedData[]) {
        return Array.from(this.getColumnData({ name: field.name, value: field }, taggedData)).map(
            (template) => [field, template] as [ModalField, TemplateResult],
        );
    }
        ...
        if (this.footer && this.buttonModal) {
            this.intersectionObserverHandler = new IntersectionObserverHandler({
                intersecting: {
                    ...
                    do: () => {
                        const bottom = parseFloat(getComputedStyle(this.buttonModal).bottom);
                        this.buttonModal.style.bottom = `${bottom + (this.footer ? this.footer.offsetHeight : 0)}px`;
                    },
                },
                intersected: {
                    ref: this.footer,
    ...
    protected *getColumnData(query: { name: string; value: ModalField }, data?: TaggedData[]) {
        const { value } = query;
            ...
            <dialog id="lms-modal" class="modal modal-bottom sm:modal-middle">
                ...
                >
                    ...
                    <div class="modal-action">
                        <button type="button" class="btn btn-secondary" @click=${this.toggleModal}>
    ...
    protected mediateChange(e: Event | CustomEvent) {
        ...
        const newFields = this.fields.map((field) => (field.name === name ? { ...field, value } : field));
