...
export default class LMSCard extends LitElement {
    ...
    override render() {
        return html`
            <div class="card shadow-md hover:relative hover:bottom-2 hover:cursor-pointer hover:shadow-lg">
                <figure>
                    <img
                        src=${this.image.src}
                        alt=${this.image.alt}
                ...
                <div class="card-body">
                    ...
                    >
                        ${this.listItems.map((listItem) => html`<li>${listItem}</li>`)}
                    </ul>
                    <div
                        class="${classMap({
                    ...
                    >
                        ${this.links.length
                            ? this.links.map((link) => html`<a href=${link.href}>${link.text}</a>`)
                            : nothing}
                    </div>
                </div>
            </div>
