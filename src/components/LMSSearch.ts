...
export default class LMSSearch extends LitElement {
    ...
    private isMacOS: boolean = /(Mac|iPhone|iPod|iPad)/i.test(navigator.userAgent);

    private boundHandleShortcut = (e: KeyboardEvent) => this.handleShortcut.bind(this)(e);
     ...
     */
    private buildQuery(query: ParsedQuery): unknown[] | Record<string, unknown> {
    ...
    override render() {
        ...
        return html`
            <div class="relative">
                ...
                />
                <span class="absolute inset-y-0 right-2 hidden items-center sm:flex">
                    <kbd
                        class="${classMap({
