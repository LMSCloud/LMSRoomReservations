...
sub BUILD {
    ...
    $self->{'_parsers'} = [
        DateTime::Format::Strptime->new(
            ...
            on_error => 'undef'
        ),    # format with timezone and seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M:%S%Z',
            on_error => 'undef'
        ),    # format with timezone and seconds
        DateTime::Format::Strptime->new(
            ...
            on_error => 'undef'
        ),    # format with seconds but no timezone
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M:%S.%6N',
            on_error => 'undef'
        ),    # format with microseconds and no timezone
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M:%SZ',
            on_error => 'undef'
        ),    # format with 'Z' indicating UTC timezone and no seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M:%S.%6NZ',
            on_error => 'undef'
        ),    # format with 'Z' indicating UTC timezone, microseconds and no seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M:%S.%3N%z',
            on_error => 'undef'
        ),
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M%z',
            on_error => 'undef'
        ),    # format with timezone and no seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M%Z',
            on_error => 'undef'
        ),    # format with 'Z' indicating UTC timezone and no seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%dT%H:%M',
            on_error => 'undef'
        ),    # format with no timezone and no seconds
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%d %H:%M:%S',
            on_error => 'undef'
        ),    # format sql-friendly with no timezone
        DateTime::Format::Strptime->new(
            pattern  => '%Y-%m-%d %H:%M',
            on_error => 'undef'
        ),    # format sql-friendly with no timezone and no seconds
    ];
...
sub is_valid {
    ...
    return ( 0, [ __('The given value is not defined.') ] )
...
sub is_valid_string {
    ...
    my $has_given_length =
        defined $args->{'length'}
        ? $args->{'value'} =~ m/^.{1,$args->{'length'}}$/smx
        : 1;
...
sub is_valid_number {
    ...
    my ( $min, $max ) =
        defined $args->{'range'} ? @{ $args->{'range'} } : ( 0, $MAX_LENGTH_INT );
    ...
    my $exceeds_max_value =
        defined $args->{'max_value'}
    ...
    my $is_in_range =
        ...
        : ( $args->{'value'} >= 0 && $args->{'value'} <= $MAX_LENGTH_INT );
...
sub is_valid_datetime {
    ...
    my $is_after_localtime =
        defined $args->{'after'}
        ? ( $args->{'value'} ge localtime->strftime('%Y-%m-%dT%H:%M:%S%z') )
        : 1;
    ...
    my $is_before =
        defined $args->{'before'} ? ( $args->{'value'} le $args->{'before'} ) : 1;
...
sub is_valid_color {
    ...
    push @{$errors},
        $given_argument
        . __(' is not a color in the format #RRGGBB or rgb(0-255, 0-255, 0-255) or rgba(0-255, 0-255, 0-255, 0.0-1.0).')
        if !$is_valid_color;
