...
sub list {
    ...
    return try {
        ...
        my $settings = [
            ...
            {   setting     => 'default_booking_time_min',
                value       => $self->retrieve_data('default_booking_time_min'),
                description => __('Minimum bookable duration (in minutes)'),
                type        => 'number',
                placeholder => __('A duration in minutes, e.g. 30'),
            },
            {   setting     => 'default_booking_time_step',
                value       => $self->retrieve_data('default_booking_time_step'),
                description => __('Booking duration step / grid (in minutes)'),
                type        => 'number',
                placeholder => __('A duration in minutes, e.g. 30'),
            },
            {   setting     => 'enforce_booking_time_grain',
                value       => $self->retrieve_data('enforce_booking_time_grain'),
                description => __('Reject OPAC bookings whose duration does not match the configured minimum and step (staff bookings remain unrestricted)'),
                type        => 'checkbox',
            },
            ...
            {   setting     => 'calendar_default_view',
                value       => $self->retrieve_data('calendar_default_view'),
                description => __('Initial calendar view mode'),
                type        => 'select',
                options     => [
            ...
            {   setting     => 'calendar_year_drill_target',
                value       => $self->retrieve_data('calendar_year_drill_target'),
                description => __('Year view click target'),
                type        => 'select',
                options     => [ { value => 'day', label => __('Day') }, { value => 'month', label => __('Month') }, ],
            },
            {   setting     => 'calendar_year_density_mode',
                value       => $self->retrieve_data('calendar_year_density_mode'),
                description => __('Year view density visualization'),
                type        => 'select',
                options     => [ { value => 'dot', label => __('Dot') }, { value => 'heatmap', label => __('Heatmap') }, { value => 'count', label => __('Count') }, ],
            },
            {   setting     => 'calendar_primary_color',
                value       => $self->retrieve_data('calendar_primary_color'),
                description => __('Calendar accent color'),
                type        => 'color',
            }
        ];
